...
There are no uninitialized variables. This is intentional:饭后 it removes a large class of UB that haunts C/C++.
...
- Typed constants can't be ranges in a union; from Go 1.18+ on, enums are better expressed using a typed set and exhaustive `switch` with `_` default checked by linters, or (`go 1.x`) the `enum` proposal under discussion.
...
for line := range func(yield func(string) bool) {} // since 1.23 preview, stable 1.26
```
...
s = append(s, 4)        // cap may grow to 6 (typically doubles up to 1024, then ~1.25x)
...
**Growth policy**: append doubles capacity up to ~1024 elements, then grows ~1.25x, per the runtime. Specific numbers aren't part of the spec but you should know there is a growth heuristic and preallocation is preferable in known-size code:
...
- `go.mod` declares the module path, Go version, requires, replaces, excludes, retract. Since Go 1.21 the `go` directive is a **minimum**; since Go 1.25 a `toolchain` directive may pin the exact toolchain (`toolchain go1.26.0`).
...
| `go fix` | Update code for API changes across Go versions |
...
- `len(nil切片)` == 0, iterating nil slice/map is a no-op (safe).
