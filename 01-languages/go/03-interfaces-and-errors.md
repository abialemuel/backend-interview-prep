...
Generics work on types (functions, types, methods restricted—Go currently doesn't allow methods with type parameters other than those of receiver), not on dynamic binding; instantiated generics are still fully statically monomorphized/shape-based GC shape stenciling in the runtime.
...
`comparable` covers primitives, pointers, channels, arrays of comparable, and structs containing only comparable fields. It **does not** cover slices, maps, functions, or interfaces — strictly comparable only. (Note: interfaces technically allow `==` — comparison compares dynamic type and value; but the language restricts `comparable` slightly to avoid silent run-time panics.)
...
- Don't wrap twice with `%w` (chain // depth ok but confusing). Wrap once at each layer.
