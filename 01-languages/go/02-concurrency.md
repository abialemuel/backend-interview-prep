...
- Number of P is `GOMAXPROCS`, default = number of CPU cores.
...
- **Timeout**: `case <-time.After(d)` (note: each call allocates a Timer and a channel; reuse a `time.Timer` if you're in a tight loop).
...
- `sync.Mutex` is non-reentrant — Locking twice from the same goroutine without Unlock deadlocks (or panics in newer runtime diagnostics).
...
Since Go 1.17, `golang.org/x/sync/semaphore` offers a weighted semaphore with nicer API; in the standard library the buffered-channel trick remains idiomatic and dependency-free.
...
- Forgetting `defer cancel()` can leak a `context.WithTimeout`'s internal timer until the timeout fires.
