...
- `opcache.jit` — mode. Common values: `1205` ("tracing", default since
  8.4) or `1235`; `disable_immortal` etc. The simpler form: `opcache.jit=on`
  / `off` as a shortcut.
  - First digit: CPU flags.
  - Second digit: register allocator optimization.
  - Third digit: JIT trigger (`0` off, `1` on first exec, `2` profile-based
    hot-path, `3` immediately).
...
- PHP 8.4 made `tracing` the default JIT mode and shipped several upstream
  performance fixes. PHP 8.5 further improved object dispatch JIT.
...
`weakref` / `WeakMap` (8.0+ `WeakMap`, 8.2+ `WeakReference` is older)
support cache and observer patterns without preventing GC.
...
- **XHProf** (original, plus Tideways / `xhprof.ext-klo` forks) — production
  sampling profiler with negligibly low overhead. Tideways is the modern
...
- **Excimer** (Wikimedia) — sampling profiler extension; good for prod
  inside large apps.
- **`zend-monitor` / Tideways / Sentry performance** — APM / tracing with
  spans across HTTP/DB/Redis.
...
- Notify ASLR/security review when shipping custom password flows; prefer
...
- 12-factor: secrets come from environment variables, not from the
  repository. Use HashiCorp Vault, AWS Secrets Manager, Doppler, etc., or the system
  keychain (iCloud keychain irrelevant, but `libsodium secretbox` of env
  files is a viable local option).
