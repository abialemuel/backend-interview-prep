...
START REPLICA;     -- was START SLAVE; (8.0.22+ aliases; 8.4 retains both)
```

Terminology note: MySQL 8.0.22+ renamed `MASTER`/`SLAVE` to `SOURCE`/`REPLICA`. Both work in 8.4; the older terms are deprecated.
...
- **Semi-synchronous** — the primary blocks the commit until *at least one* replica has acknowledged receipt of the binlog event (after `rpl_semi_sync_master_wait_for_slave_count`, default 1). If the timeout expires it falls back to async. Two variants:
...
- **UUID v7** (time-ordered, RFC 9562): the first 48 bits are a Unix timestamp in ms, the rest random. Inserts are roughly monotonic → comparable locality to auto-increment while keeping global uniqueness. MySQL 8.4 does not ship a native UUIDv7 generator; generate in the app or via `UUID()` reordering tricks.
