...
**PostgreSQL contrast** (frequent follow-up): PostgreSQL defaults to READ COMMITTED, its REPEATABLE READ is a pure snapshot level with *no gap locks* (writes that would break the snapshot fail with a serialization error instead of blocking), and its SERIALIZABLE uses Serializable Snapshot Isolation (SSI) — optimistic conflict detection rather than InnoDB's pessimistic next-key locking. Same SQL-standard names, very different mechanics. See [PostgreSQL architecture and MVCC](../postgresql/01-architecture-and-mvcc.md).
...
Note the architectural contrast with PostgreSQL: InnoDB keeps old versions *out of line* in the undo log and updates rows in place, so purge lag bloats the undo tablespace; PostgreSQL keeps old versions *in the heap* as dead tuples, so the equivalent problem is table bloat cleaned by VACUUM. Both systems share the same failure mode — a long-running transaction pins old versions — they just pay for it in different places.
