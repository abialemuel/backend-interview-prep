...
Indexes are the single most impactful lever for MySQL performance and the most common interview topic. This file covers the InnoDB index model end-to-end, how the optimizer chooses indexes, how to read `EXPLAIN`, and the practical pitfalls (functions in `WHERE`, leading wildcards, `OFFSET` pagination, etc.). Everything here applies to MySQL 8.0 through 8.4 LTS and 9.x/9.7 LTS unless a version is called out.

One historical trap up front: the **query cache was removed in MySQL 8.0**. It serialized on a global mutex and invalidated on any write to a touched table, so it hurt more than it helped on real workloads. If a question or an old article treats `query_cache_size` as a tuning lever, the correct answer is "that feature no longer exists — result caching belongs in the application layer (e.g. Redis)."
...
Also know that MySQL 8.0.18+ has a **hash join** for equi-joins where no usable index exists on the join column — `EXPLAIN FORMAT=TREE` shows `Inner hash join`. It replaced the old block nested-loop join and makes unindexed analytical joins far less catastrophic, but an indexed nested-loop join (`eq_ref`/`ref`) is still what you want on OLTP paths.
...
One nuance worth knowing for senior interviews: since 8.0.13 the optimizer has a **skip scan** — if the leading column has *very* low cardinality (e.g. `gender`), a query filtering only on `b` can be executed as one range scan per distinct value of `a`. It shows up as `Using index for skip scan` in `Extra`. It is a narrow escape hatch, not a reason to design indexes with the wrong leading column.
...
### Invisible indexes: drop safely (8.0+)

Dropping an index is scary — if some rare-but-critical query depended on it, rebuilding a large index takes hours. MySQL 8.0+ lets you make an index **invisible**: it is still maintained on every write but the optimizer ignores it.

```sql
ALTER TABLE orders ALTER INDEX idx_user_status_created INVISIBLE;
-- watch the workload (slow log, p95 latency) for a few days...
ALTER TABLE orders ALTER INDEX idx_user_status_created VISIBLE;   -- instant rollback
ALTER TABLE orders DROP INDEX idx_user_status_created;            -- when confident
```

This is the standard 2026-era answer to "how do you decommission an index safely in production" — toggle invisible, observe, then drop. (You can also flip `optimizer_switch='use_invisible_indexes=on'` per session to test a *new* index on live traffic before exposing it globally.)
...
- Decommission indexes via `INVISIBLE` first, then drop.

**PostgreSQL contrast** (a common follow-up): PostgreSQL tables are heaps, not index-organized — every index (including the PK) points to a physical tuple location, so there is no clustered-index/bookmark-lookup asymmetry, but also no "free" PK ordering; and PostgreSQL offers more index types (GIN, GiST, BRIN, partial and expression indexes as first-class citizens). See [PostgreSQL indexing and performance](../postgresql/02-indexing-and-performance.md).