...
Model answers are written for an InnoDB / MySQL 8.4 LTS context (with 9.x/9.7 LTS additions noted; 8.0 reached EOL in April 2026). Where the answer is engine-specific, that is called out.

**How questions are graded.** Interviewers typically calibrate like this:

- **Junior** — correct definitions and the ability to read `EXPLAIN`; knows the "what".
- **Senior** — mechanisms and trade-offs: *why* InnoDB behaves this way, what breaks at scale, and how to fix it safely in production.
- **Staff** — architecture decisions under constraints: replication topology and RPO/RTO trade-offs, migration strategy, when MySQL is the wrong tool (and being able to argue [MySQL vs PostgreSQL](../postgresql/04-interview-questions.md) both ways).

The sections below map roughly: Easy → junior baseline, Medium → senior, Hard → senior/staff. A staff-level answer to an "easy" question still differs from a junior one — it adds the failure modes and the version history unprompted.
...
In **asynchronous** replication (the default) the primary commits as soon as its own binlog is written; the replica may be arbitrarily behind. Primary failure can lose transactions the replica had not yet received — RPO equals lag at the moment of failure. In **semi-synchronous** replication the primary blocks the commit until at least one replica (configurable via `rpl_semi_sync_source_wait_for_replica_count`) acknowledges receipt of the binlog event; if the timeout fires it degrades back to async. With `AFTER_SYNC` (default in 8.0+) the wait happens before the primary commits, so no committed transaction is lost on failover (though a duplicate may appear if the replica received but the primary did not commit). Semi-sync gives near-zero RPO at the cost of one round-trip per commit; async gives the lowest write latency but the weakest durability.
...
### Q31: What are invisible indexes, and how do you use them to drop an index safely? *(senior)*

An invisible index (MySQL 8.0+) is fully maintained on every write but ignored by the optimizer. It exists to decouple "stop using this index" from "destroy this index." The safe decommissioning workflow: `ALTER TABLE t ALTER INDEX idx INVISIBLE`, then watch the workload (slow log, `events_statements_summary_by_digest`, p95 latencies) for days — long enough to cover weekly and monthly batch jobs. If something regresses, `ALTER ... VISIBLE` restores it instantly; if nothing does, `DROP INDEX` for real. The reverse direction also works: create a new index, keep it invisible, and test it against production queries in one session with `SET optimizer_switch='use_invisible_indexes=on'` before exposing it to all traffic. A staff-level addition: this matters because index builds on large tables take hours, so a mistaken drop is an incident, not an undo.

### Q32: How does the clone plugin change how you provision a new replica? *(senior)*

Before 8.0.17 you provisioned replicas from backups: take an XtraBackup/mysqldump, restore, figure out binlog coordinates or GTID purge sets, then start replication — slow and error-prone. The clone plugin does a physical, page-level copy of the InnoDB data from a running donor directly over the network: `CLONE INSTANCE FROM user@donor:3306 ...` on the recipient replaces its entire data directory with a consistent snapshot of the donor, *including* the donor's `GTID_EXECUTED`. After the automatic restart you just run `CHANGE REPLICATION SOURCE TO ... SOURCE_AUTO_POSITION=1` and the replica catches up. The donor stays online during the clone (progress is visible in `performance_schema.clone_progress`). It is also what InnoDB Cluster uses under the hood to auto-provision new group members. Mentioning "clone plugin, then GTID auto-position" is the current expected answer to "a replica died, how do you rebuild it?"

### Q33: You need HA with zero data loss on failover. Semi-sync or Group Replication? *(staff)*

First clarify "zero": semi-sync with `AFTER_SYNC` gives RPO≈0 **only while the guarantee holds** — if the replica ack times out (`rpl_semi_sync_source_timeout`), the primary silently degrades to async and keeps committing, so under exactly the network stress that precedes a failover, your zero-loss guarantee may have evaporated. Group Replication makes the guarantee hard: a transaction is not committed until a quorum has certified it, and membership changes are handled by the group protocol itself, so a promoted member is guaranteed to have every committed transaction. The price: stricter requirements (every table needs a PK, ROW binlog, GTID), flow control that throttles the whole group to its slowest member, sensitivity to inter-node RTT, and more moving parts (usually deployed as InnoDB Cluster with MySQL Router). Staff-level framing: semi-sync is a bolt-on that weakens under stress but is trivially retrofitted; MGR is a consensus system with a real guarantee that you must design the deployment around. If the business requirement is genuinely "no committed transaction may be lost," semi-sync's fallback-to-async behavior disqualifies it unless you also alert on degradation and treat it as an incident.

### Q34: A teammate suggests enabling the MySQL query cache to fix slow reads. What do you say? *(junior/senior filter)*

The query cache no longer exists — it was deprecated in 5.7 and **removed entirely in MySQL 8.0**. It was removed because it was a global serialization point (every read checked a mutex-protected cache) and its invalidation was per-table: any write to a table wiped every cached result touching that table, so on realistic read/write mixes it cost more than it saved. The modern layering: let InnoDB's buffer pool cache pages (that is the in-database cache that matters — size `innodb_buffer_pool_size` correctly), fix queries with indexes and `EXPLAIN`, and put result caching where invalidation can be controlled — in the application with Redis or Memcached (see [Redis caching patterns](../redis/02-persistence-and-clustering.md)). This question is a filter: candidates repeating 2010s blog advice fail it.

### Q35: How would you choose between MySQL and PostgreSQL for a new service in 2026? *(staff)*

Both are excellent general-purpose OLTP databases, so a staff answer is about fit, not fandom. Reasons to pick MySQL: clustered-index storage is ideal for PK-ordered access patterns; replication and the tooling ecosystem around it (gh-ost, Vitess for sharding, decades of operational lore) are unmatched; connection handling is cheap (thread per connection vs process per connection — PostgreSQL usually needs PgBouncer); InnoDB's undo-based MVCC avoids VACUUM-style bloat management. Reasons to pick PostgreSQL: richer SQL and types (arrays, range types, real `CHECK` enforcement history, transactional DDL — MySQL DDL still implicitly commits); more index types (GIN/GiST/BRIN, partial and expression indexes); extensions as a superpower (PostGIS, pgvector for embeddings, TimescaleDB); true SERIALIZABLE via SSI. Also compare defaults honestly: MySQL runs REPEATABLE READ with gap locks, PostgreSQL runs READ COMMITTED with no gap locks — different anomaly and deadlock profiles out of the box. Team experience and existing operational muscle usually dominate the marginal technical differences. See the [PostgreSQL section](../postgresql/README.md) for the other side's depth, and [PostgreSQL vs MySQL MVCC](../postgresql/01-architecture-and-mvcc.md) for the mechanism-level contrast interviewers love.