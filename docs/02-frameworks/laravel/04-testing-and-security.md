...
Laravel ships with PHPUnit support, and new applications created via the Laravel installer default to **Pest** (you can pick PHPUnit at install time; both remain fully supported). The `Tests\TestCase` is still the base; Pest provides a thin function layer over it, so everything below applies to both.
...
- **`DatabaseTruncation`** (10.x+) truncates tables between tests for cases where transactions don't work (e.g., concurrent connections, Dusk, or DDL inside tests).
...
The CSRF middleware (in the `web` middleware group) rejects POST/PUT/DELETE without a valid `_token` matching the session. In Laravel 13 this was formalized as `PreventRequestForgery`, which adds **origin-aware verification** (checking `Origin`/`Sec-Fetch-Site` headers) on top of the classic token check. For APIs, the `api` group does not include CSRF middleware because API clients are typically stateless tokens.

Exempt specific routes from CSRF (rarely needed — webhooks mostly; use sparingly):
...
`Illuminate\Encryption\EncryptionServiceProvider` binds `Illuminate\Contracts\Encryption\Encrypter` backed by OpenSSL AES-256-CBC (with an HMAC over the ciphertext; AES-256-GCM is also supported). `APP_KEY` is the encryption key; never expose it.
...
Encrypted casts in Eloquent: `'ssn' => 'encrypted'`, `'settings' => AsEncryptedArrayObject::class`. Use these for sensitive columns at rest. Since Laravel 11, key rotation is graceful: list old keys in `APP_PREVIOUS_KEYS` (comma-separated) and Laravel decrypts with them while encrypting new values with the current `APP_KEY` — no big-bang re-encryption required, though re-encrypting lazily is still good hygiene.
...
- **bcrypt** — default; rounds default to 12 since Laravel 11 (was 10), tunable via `BCRYPT_ROUNDS`.
- **argon2id** — memory-hard, the stronger modern choice; requires PHP compiled with Argon2 support.
- `Hash::needsRehash()` on login lets you transparently upgrade stored hashes when you raise the cost.
- Never store plain hashes of passwords; never roll your own.

### Mass assignment

`$fillable` whitelist or `$guarded` blacklist prevents unintended column writes during `::create`/`->fill`/`->update`. With `$guarded = []` you accept the risk. `Model::preventSilentlyDiscardingAttributes()` (10.x+) is the strict-mode counterpart to `preventLazyLoading`: it throws when a mass-assigned attribute is not fillable instead of silently dropping it — enable it outside production to catch mistakes early. `Model::shouldBeStrict()` enables it together with lazy-loading and missing-attribute protection in one call.
...
1. User submits email to `/forgot-password` → broker creates a hashed token in `password_reset_tokens`.
...
- 2FA (TOTP) ships first-party via `laravel/fortify`, which the official starter kits use under the hood.
- `password.confirm` middleware forces re-entry of password before sensitive actions.
- Sanctum tokens can be given a global lifetime (`sanctum.expiration`) or per-token `expires_at`; prune stale tokens with the `sanctum:prune-expired` scheduled command.
- **Precognition** (first-party) runs a form request's validation rules for "live" frontend validation without executing the controller — same rules, no duplication in JS.
