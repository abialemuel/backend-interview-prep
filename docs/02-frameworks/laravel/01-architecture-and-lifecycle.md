...
public/index.php
   └── bootstrap/app.php            (creates the Application, binds kernels & handler)
        └── HTTP Kernel             (framework-internal in Laravel 11+, configured in bootstrap/app.php)
...
The Application instance is created in `bootstrap/app.php`, which since Laravel 11 (and unchanged through 12 and 13) is where middleware, exception handling, routing, and console/commands configuration lives instead of in dedicated kernel classes:
...
Providers are the place to register container bindings, event listeners, observers, custom validation rules, and Artisan commands. Everything else (routes, middleware) lives in `bootstrap/app.php` and `routes/`.
...
Since Laravel 11, middleware is configured in `bootstrap/app.php` via the `withMiddleware` closure rather than in `app/Http/Kernel.php`:
...
### Controller middleware via attributes (Laravel 13)

Laravel 13 added first-party PHP attributes so middleware and authorization can be declared directly on controllers instead of in route files:
...
The older `HasMiddleware` interface with a static `middleware()` method (introduced in 11) still works; attributes are the more declarative 13.x style. Both replace the removed `$this->middleware(...)` constructor calls from Laravel 10 and earlier.
...
## Octane and the long-lived application

Octane serves your app from a persistent worker (FrankenPHP, Swoole, or RoadRunner) instead of booting the framework per request. Boot happens once per worker; each request then reuses the already-booted application. That is where the ~2-10x throughput win comes from — and where the interview gotchas live:

- **Singletons persist across requests.** A singleton that captures request state (the current user, a `Request` instance, a config value derived from the request) will leak that state into the *next* request served by the same worker. Fixes: use `scoped()` instead of `singleton()` for per-request state, inject dependencies lazily (resolve inside the method, or bind a closure), or reset state in an Octane listener (`RequestReceived`).
- **Never inject the container or `Request` into a constructor of a singleton.** You will hold a stale reference. Resolve them at call time.
- **Static properties and memoization accumulate.** Static caches grow unbounded across thousands of requests; memory leaks that were invisible in php-fpm become worker-killers. Octane's `--max-requests` recycles workers as a safety net.
- **`config()` mutations persist.** Runtime `config(['x' => ...])` changes bleed across requests unless reset.

Octane's `octane:reload` must run on deploy (analogous to `queue:restart`) so workers pick up new code. If you cannot audit your app for state leaks, plain php-fpm remains a perfectly good default — Octane is an optimization, not a requirement.
