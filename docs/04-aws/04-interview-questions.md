...
**How these are graded.** Difficulty maps to the level bar:

- **Easy = junior bar.** Everyone is expected to answer; juniors pass by
  stating the facts, seniors are expected to add the trade-off unprompted.
- **Medium = senior bar.** A senior answer names the decision criteria and
  at least one failure mode ("and here's where this bites you"); a junior
  who gets partway through these is doing well.
- **Hard = staff bar.** Graded on structure: clarifying requirements first
  (scale, latency, RPO/RTO, budget), naming 2–3 viable designs, picking one
  with explicit trade-offs across Well-Architected pillars, and covering
  cost + operations. A staff answer also says what they'd *not* build.

Across all levels, the differentiator is not memorized limits — it's
whether you volunteer *when a choice is wrong*.
...
the generation matters: `8g` is Graviton4 ARM (`c8g`/`m8g`/`r8g`/`x8g`/
`i8g`), `8i` is the custom Intel Xeon 6 generation, `a` is AMD — Graviton
typically gives ~20% lower price and often +10-20% perf (25-40% better
price/perf) for interpreted/JIT and compiled workloads (Go, Java, Python,
Node, Rust).
...
the surplus credits. Use t-instances for dev, low-
...
the default on new AMIs/launch templates, Security Hub/CIS flag v1 as a
finding, and you can enforce v2 region-wide via IMDS defaults. SSRF still
matters — even with v2, scope your instance role narrowly.

### Q5: Summarize S3 consistency guarantees.

**Answer:** Since December 2020 S3 is **strongly consistent**: strong
read-after-write for PUTs of new objects, overwrite PUTs, and DELETEs — any
subsequent GET returns the latest version — **and list operations reflect
completed writes too**. The classic "write then immediately read back from a
different process" no longer needs a retry-on-404 dance. Two things remain
asynchronous: cross-region replication, and cache layers you add yourself
(CloudFront). Bonus point for 2026: S3 now also supports **conditional
writes** (`If-None-Match`/`If-Match` ETag preconditions), giving you
compare-and-swap semantics for multi-writer coordination.
...
particularly heavy for Java/.NET (long classloading/JIT). Since **August
2025 the INIT phase is billed**, so cold starts cost money as well as
latency. Mitigations, in order of impact: shrink the deployment package
(remove heavy deps, use layers for shared code), preload SDKs and DB
clients in global scope but keep init small, **SnapStart** (restore from a
snapshot of the post-init environment — now **Java, Python 3.12+, and
.NET 8+**; free for Java, priced for the others; eliminates most cold-start
latency), **Provisioned Concurrency** (keep N environments warm and
pre-init, paid hourly regardless of invocations), or use lighter runtimes
(Node/Go/Rust cold-start in tens of ms). Care: Provisioned Concurrency
nullifies the "pay only when invoked" cost model, and SnapStart has caveats
(uniqueness of random seeds/connections at restore, no EFS) — measure first.
...
**Answer:** Aurora Serverless v2 scales compute between **0 and 256 ACU**
(each ACU ≈ 2 GB RAM + proportional vCPU) up and down in seconds based on
load — including **auto-pause to zero** when idle with resume on first
connection (~15 s, so not for latency-critical always-on paths) — and you
can mix Serverless v2 with provisioned instances in the same cluster. It's
great for variable / multi-tenant workloads where load swings dramatically
(dev/test that runs hot during work hours and idle otherwise,
...
per request (`ReadRequestUnits` / `WriteRequestUnits`) with no capacity
planning; after the **November 2024 ~50% on-demand price cut** it's the
recommended default — provisioned only clearly wins at sustained, high,
predictable utilization. Two extra levers: **adaptive capacity**
automatically reallocates throughput between partitions of a busy table,
and **warm throughput** lets you pre-warm an on-demand table before a
known traffic spike instead of relying on ramp-up.
...
common in real systems. 2026 wrinkle: **Aurora DSQL** blurs the line —
serverless, Postgres-compatible, multi-region active-active — so "I need
relational *and* serverless scale" no longer auto-selects DynamoDB, though
DSQL's Postgres subset and optimistic concurrency are real constraints.
...
**Answer:** AWS Graviton (Graviton4 is current: `c8g`, `m8g`, `r8g`, `x8g`,
`i8g`; `t4g` for burstable) is ARM — typically **~20% lower price** AND often
**+10-20% performance** vs equivalent Intel/AMD, giving roughly **25-40%
better price/perf** for compatible workloads (Go, Rust, Java on JDK 17+,
Node, Python), plus ~60% less energy per unit of work (Sustainability
pillar). The story extends past EC2: Lambda arm64 is ~20% cheaper per
GB-second, and Fargate, RDS/Aurora, ElastiCache, and OpenSearch all offer
Graviton. Caveats: native deps need ARM builds (e.g., libpq, image
libraries), CI matrix needs an ARM lane, some vendors still ship x86-only
images. The pattern: rebuild CI images multi-arch, A/B the prod workload
against the x86 baseline for a week, and migrate if perf and cost both
improve — most stateless HTTP services do.
...
If asked "and if I *must* have zero RPO across regions?" — the 2026 answers
are DynamoDB Global Tables with **multi-Region strong consistency** or
**Aurora DSQL**, both trading write latency for synchronous cross-region
durability.
...
### Q44: Design a serverless API for a consumer app and defend the cold-start story.

**Answer:** Baseline: API Gateway HTTP API → Lambda → DynamoDB (on-demand),
with Cognito/JWT auth, CloudFront in front for edge caching/TLS, and
EventBridge for async side-effects. Then attack your own cold-start story
before the interviewer does: (1) quantify it — cold starts hit only new
execution environments, so at steady traffic the p99 impact is small; the
pain is bursts and low-traffic paths; (2) engineer init down — small
packages, lightweight runtime (Node/Go/Rust), lazy-init heavy clients;
(3) for JVM/.NET/Python use **SnapStart**; (4) for hard p99 SLAs, put
**Provisioned Concurrency** on the hot aliases, sized from traffic curves
(scheduled scaling for diurnal load); (5) note that since Aug 2025 INIT time
is billed, so init hygiene is also cost hygiene. Close with the honest exit:
if the service is high, steady throughput with strict tail latency, a
Fargate service behind an ALB is cheaper and flatter — Lambda earns its keep
on spiky/sparse traffic and ops-free scaling.

**Grading:** *Junior* — correct service chain and knows what a cold start
is. *Senior* — quantifies cold-start exposure, names SnapStart vs
Provisioned Concurrency and their cost trade-offs, mentions DLQs/idempotency
for the async path. *Staff* — leads with SLOs and traffic shape, prices the
design vs a container baseline, and states the crossover point where they'd
abandon Lambda.

### Q45: S3 vs EBS vs EFS — a service needs storage; walk me through the decision.

**Answer:** Ask what the access pattern is, then map: **EBS** is a block
device for **one instance** (or a small multi-attach cluster) in one AZ —
lowest-latency random I/O; it's the answer for databases and boot volumes;
snapshots give durability. **EFS** is a shared POSIX filesystem (NFS)
mountable by **many instances/containers/Lambdas across AZs**, elastic
capacity and pay-per-GB — the answer for shared content, user uploads on a
fleet, legacy apps that "need a folder"; per-op latency is ms, so it's wrong
for databases and small-file churn. **S3** is object storage over HTTP —
11-nines durable, unlimited scale, lifecycle tiering, strongly consistent,
but no partial in-place writes and no POSIX — the answer for blobs, backups,
static assets, and data-lake content; add **S3 Express One Zone** when you
need single-digit-ms object access co-located with compute. Heuristic:
"random R/W attached to one host" → EBS; "shared tree across a fleet" → EFS;
"write-once blobs at scale, cheapest, most durable" → S3.

**Grading:** *Junior* — correct one-liner per service. *Senior* — leads
with access pattern, latency, and sharing requirements; knows EFS's latency
caveat and S3's non-POSIX model. *Staff* — adds cost per GB vs per-request
economics, FSx for the Windows/Lustre gaps, and failure/AZ semantics
(EBS is AZ-local; EFS multi-AZ; One Zone variants trade that away).

### Q46: How do you structure AWS accounts for a growing organization?

**Answer:** Accounts are the real isolation, billing, and quota boundary —
so use many. AWS Organizations with a workload-free **management account**;
OUs for `Security`, `Infrastructure`, `Workloads/Prod`, `Workloads/NonProd`,
`Sandbox`; dedicated **Log Archive** and **Security Tooling** accounts
(delegated admin for GuardDuty/Security Hub/Access Analyzer, org CloudTrail
into locked-down S3). One account per workload per environment. **SCPs** on
OUs as guardrails (deny non-approved regions, deny disabling CloudTrail,
protect the logging bucket); **IAM Identity Center** for all human access
with permission sets (no IAM users); **centralized root access management**
to remove member-account root creds. Provision via Control Tower account
factory or an IaC landing zone. Networking across accounts: shared-VPC via
RAM or a central network account with Transit Gateway; service-to-service
via VPC Lattice/PrivateLink. Trade-off to volunteer: many accounts add
provisioning and cross-account-debugging friction — the guardrails and
automation are the price of blast-radius containment.

**Grading:** *Junior* — knows Organizations exists and prod/non-prod should
be separated. *Senior* — OU layout, SCPs vs IAM, Identity Center, log
archive/security accounts. *Staff* — account-per-workload rationale (blast
radius, quotas, cost attribution), account-vending automation, network
topology across accounts, and how the structure evolves (start with 4–5
accounts, not 50).

### Q47: Your AWS bill doubled month-over-month. Walk me through finding and fixing it.

**Answer:** Triage first: Cost Explorer grouped by **service**, then by
**linked account/tag/usage type**, comparing the two months — most doublings
are one service. Check Cost Anomaly Detection findings. Usual suspects, in
order: data transfer (NAT Gateway per-GB processing — a chatty private
subnet talking to S3 without a gateway endpoint; cross-AZ traffic; egress),
CloudWatch Logs ingestion (debug logging left on), unlifecycled S3
(versioned buckets accumulating noncurrent versions, incomplete multipart
uploads), on-demand DynamoDB/Lambda hit by a retry storm or infinite event
loop, GPU/large instances left running in dev, and cross-region replication
you forgot you enabled. Fix mechanically, then structurally: budget alarms
with per-team scoping, tagging enforced via SCP/tag policies, CUR + Athena
dashboards for unit economics, Savings Plans only *after* usage stabilizes,
and guardrails (auto-stop non-prod, log retention policies, S3 lifecycle by
default). Staff-level extra: assign each service a cost-per-request /
cost-per-tenant metric so regressions surface in review, not in the invoice.

**Grading:** *Junior* — opens Cost Explorer, finds the service. *Senior* —
names the classic silent costs (NAT processing, CW Logs, versioned S3) and
fixes plus alarms. *Staff* — unit-economics framing, prevention via org
guardrails, and the discipline of commitment purchases after right-sizing,
never before.

### Q48: A product needs low-latency reads and writes for users in the US, EU, and APAC. Compare your multi-region data options.

**Answer:** First clarify consistency needs and write patterns — that
decides everything. Options: (1) **DynamoDB Global Tables (classic)** —
active-active, local reads *and writes* everywhere, sub-second async
replication, last-writer-wins conflicts; great for profile/session-style
data that tolerates rare conflict overwrites. (2) **DynamoDB Global Tables
with multi-Region strong consistency** — RPO zero and read-your-writes from
any region, at the cost of cross-region write latency. (3) **Aurora Global
Database** — relational, one writer region + fast read-only secondaries
(<1 s lag), so EU/APAC writes pay a trip to the writer region; managed
failover for DR. (4) **Aurora DSQL** — Postgres-compatible active-active
multi-region writes with strong consistency and optimistic concurrency;
newest option, Postgres-subset and retry-on-conflict constraints. (5)
Pragmatic hybrid: partition tenants by home region (data residency often
forces this anyway) and only globalize the small truly-shared dataset.
Volunteer the costs: cross-region replication traffic, doubled+ storage,
and a failover runbook that must actually be tested.

**Grading:** *Junior* — knows Global Tables and read replicas exist.
*Senior* — maps consistency requirements to the async/sync trade-off and
picks per-dataset, not per-app. *Staff* — brings up data residency,
tenant-partitioning as the cheaper alternative to global strong
consistency, conflict semantics, and tested failover.