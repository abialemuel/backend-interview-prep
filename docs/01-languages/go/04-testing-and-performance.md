...
func BenchmarkAdd(b *testing.B) {
    for b.Loop() {          // Go 1.24+: the preferred form
        add(1, 2)
    }
}

func BenchmarkAddLegacy(b *testing.B) {
    for i := 0; i < b.N; i++ {  // classic form; still everywhere in existing code
        add(1, 2)
    }
}
```

`b.N` is grown by the framework until a stable duration is measured (~1s); the body must process the entire `b.N`-sized workload. **`b.Loop()` (Go 1.24)** is the modern replacement: it does the same scaling, automatically excludes setup done before the loop, and keeps the loop body's parameters and results alive so the compiler cannot optimize the work away — eliminating the traditional "assign to a package-level sink" trick. (Since Go 1.26 `b.Loop` no longer inhibits inlining inside the body, removing its last measurement distortion.) Know both forms.
...
- **Compiler optimization elision** (legacy `b.N` loops): if the result of your inner work is unused, the optimizer may eliminate it. Assign to a package-level sink — or just use `b.Loop()`, which keeps results alive for you:
  ```go
  var Sink int
...
`httptest.NewServer` is the standard way to fake outbound HTTP: it binds a real listener on localhost and returns its URL for the code under test.
...
Coverage is counted at the **statement** granularity. 70–90% is typical; aim for high coverage on critical business logic, not right at boilerplate. Since Go 1.20, `go build -cover` instruments whole binaries, so **integration tests** (run the real binary, collect `GOCOVERDIR` data) count toward coverage too — worth mentioning when asked how you'd measure coverage beyond unit tests.
...
Since Go 1.26 the pprof web UI (`go tool pprof -http=:8080 profile.pb.gz`) opens on the **flame-graph view** by default, with the classic call graph under View → Graph. Choosing the right sample index (`-sample_index=inuse_space` vs `alloc_space`) is still the most important knob when reading heap profiles.
...
For production, **`trace.FlightRecorder`** (Go 1.25) continuously records into an in-memory ring buffer holding the last few seconds of execution; call `WriteTo` when something interesting happens (a request blows its SLO) to snapshot just that window. Vastly cheaper than tracing everything, and the modern answer to "how would you debug a rare latency spike in prod?"
...
Learn to read the output: `./x.go:10:7: x escapes to heap`. Recent releases keep chipping at this: Go 1.25 and 1.26 let the compiler stack-allocate the **backing store of slices** in more situations (including some variable-size `make` calls), so code that used to heap-allocate small temporary slices often no longer does. Don't memorize the exact cases — say "escape analysis improves release to release; I verify with `-gcflags=-m` and benchmarks."
...
**Green Tea GC**: introduced experimentally in Go 1.25 and **enabled by default in Go 1.26**, the redesigned garbage collector scans memory with much better locality and CPU scalability — 10–40% lower GC overhead on GC-heavy workloads, with no API change and no tuning knob. The interview-ready summary: `GOGC` controls *when* the GC runs, `GOMEMLIMIT` caps *how big* the heap may get, and Green Tea reduced *what each cycle costs*.
...
## testing/synctest (stable since Go 1.25)

`testing/synctest` (experimental behind `GOEXPERIMENT=synctest` in 1.24, stable since 1.25) provides a "fake clock" goroutine sandbox (a *bubble*). Inside `synctest.Test(t, func(t *testing.T))`, time is virtualized: the clock advances only when every goroutine in the bubble is durably blocked, so sleeps and timeouts resolve instantly and deterministically. `synctest.Wait()` blocks until all bubble goroutines are idle — handy for asserting state between phases. This tests time-dependent code without real sleeping:
...
2. Know both benchmark loop forms — `for b.Loop()` (1.24+, self-defending against dead-code elimination) and the legacy `b.N` loop with its sink/ResetTimer rituals.
...
6. Know preallocation, `strings.Builder`, `sync.Pool`, `GOGC`/`GOMEMLIMIT`, and the one-liner on Green Tea GC (default since 1.26).
7. Know `testing/synctest` (1.25) for deterministic time-dependent tests and `trace.FlightRecorder` (1.25) for production latency forensics.