...
- **Easy — junior.** Any candidate should answer these cold; a senior should answer them in two sentences and volunteer the underlying mechanism.
- **Medium — senior.** Expected depth for 4+ years of Go; interviewers probe trade-offs ("when would you *not* do this?").
- **Hard — senior/staff.** A staff-level answer connects the mechanism to production consequences: costs, failure modes, migration paths, and what they'd flag in code review. The difference between senior and staff is rarely more facts — it's *when-not-to* knowledge.
- **New in 2025–2026** (final section) — questions top-tier interviewers added recently; levels are marked per question.
...
The race detector doesn't catch leaks; `go.uber.org/goleak` detects them at test teardown, production servers should expose `/debug/pprof/goroutine`, and Go 1.26 adds an experimental `goroutineleak` pprof profile that reports goroutines blocked on unreachable primitives.

### Q24: How do you detect data races?
**Answer:** Build/test with `-race`: the Thread Sanitizer instrumented build catches races actually executed during the run and reports both goroutines' stacks. Run it in CI on every PR. It only reports races **observed** in the run, so high-coverage tests help; it's not exhaustive. Instrumentation is per-binary, not per-request — so the production variant is running a small fraction of *canary instances* built with `-race` (5–10x CPU/memory cost) to catch rare interleavings that tests never hit.
...
**Answer:** Panic is appropriate for genuinely unrecoverable conditions: programmer-bug invariants, "this should never happen" impossible states. It's also appropriate for short-circuiting deeply nested logic where you immediately abort, but then `recover` should be at a top-level boundary (e.g., an HTTP handler) so the goroutine survives. `recover` must be called **in a deferred function of the same goroutine** — a deferred in another goroutine won't catch the panic of the first. Example:
...
### Q42: How do range-over-func iterators work, and when would you reach for them? *(senior)*
**Answer:** Since Go 1.23, `range` accepts a function of type `iter.Seq[V]` — `func(yield func(V) bool)` — or `iter.Seq2[K, V]`. The loop body becomes the `yield` callback; `break`/early return makes `yield` return `false`, which the iterator must honor by stopping. `slices` and `maps` expose iterator helpers (`slices.Values`, `maps.Keys`), and `iter.Pull` converts a push iterator into a `next`/`stop` pair for interleaving sequences. Use them for lazy pipelines over large or streaming data (avoid materializing intermediate slices), for exposing traversal of a custom container without leaking its internals, and anywhere you'd previously return a channel just to iterate (channels-for-iteration cost scheduling and can leak goroutines; iterators are synchronous and leak-free). A staff-level addendum: iterators are push-based and run in the consumer's goroutine — no concurrency is implied, and a `defer` inside the iterator runs when iteration ends, giving clean resource scoping.

### Q43: How would you structure logging in a new Go service? *(senior)*
**Answer:** `log/slog` (Go 1.21) is the standard answer: structured key-value records through a `Logger` front-end and a pluggable `Handler` back-end (`JSONHandler` for production, `TextHandler` for dev, `NewMultiHandler` in 1.26 to fan out to both). Set a process-wide default with `slog.SetDefault`, derive request-scoped loggers with `logger.With("request_id", id)`, and pass `ctx` via `InfoContext` so a custom handler can attach trace IDs. In hot paths use `LogAttrs` with typed `slog.String/Int` attributes to avoid `any` boxing. Levels are handler-side, so you can raise verbosity per handler without touching call sites. And the classic follow-up — "where do you log errors?" — once, at the layer that handles the error; layers below wrap with `%w` and return.

### Q44: You review a PR that stores a `context.Context` in a struct and uses `context.WithValue` to pass the DB pool. What do you say? *(senior/staff)*
**Answer:** Both are misuses. Storing a context in a struct ties the object to one call's lifetime and hides cancellation flow — contexts should be per-call parameters (first argument, by convention); the rare exceptions (`http.Request`) are containers for a single request by definition. `WithValue` carrying a DB pool is dependency injection through an untyped bag: it defeats compile-time checking, hides the dependency from the signature, and couples callers to key conventions — constructors should receive the pool. `WithValue` is only for cross-cutting, request-scoped metadata (trace ID, auth principal) under unexported key types. Staff-level extras: check that background work spawned from the request uses `context.WithoutCancel` (1.21) rather than `context.Background()` so tracing values survive but request cancellation doesn't kill it, and that every `WithTimeout` has a `defer cancel()` (vet's `lostcancel`).

### Q45: Your Go service runs in Kubernetes with a CPU limit of 2 and a memory limit of 1 GiB. What runtime settings matter? *(staff)*
**Answer:** Two knobs plus one freebie. **GOMAXPROCS**: since Go 1.25 the runtime is container-aware on Linux — it derives `GOMAXPROCS` from the cgroup CPU quota (2, here) instead of the node's core count, and updates it if the limit changes; on older Go you needed `uber-go/automaxprocs`, and getting this wrong meant dozens of Ps generating scheduler churn and CFS throttling. **GOMEMLIMIT** (1.19): set it to ~85–90% of the 1 GiB cgroup limit so the GC compresses the heap before the kernel OOM-kills the pod; pair with `GOGC` (possibly `GOGC=off` plus a limit, for pure limit-driven collection) depending on throughput vs memory trade-offs. The freebie: Go 1.26 enables the Green Tea collector by default — 10–40% less GC CPU on allocation-heavy services with zero configuration. Mentioning that memory *limit* handling is soft (GC effort ramps, it never hard-stops allocation) distinguishes a staff answer.

### Q46: How do you deterministically test code that uses timers, tickers, or `context.WithTimeout`? *(senior/staff)*
**Answer:** Historically: inject a clock interface (`clockwork`, `benbjohnson/clock`) or accept flaky `time.Sleep`-based tests. Since Go 1.25 the standard answer is `testing/synctest`: `synctest.Test(t, func(t *testing.T) {...})` runs the function in a *bubble* with a virtual clock that only advances when every goroutine in the bubble is durably blocked — so a 3-second backoff test completes in microseconds and is fully deterministic. `synctest.Wait()` lets you assert intermediate state once all goroutines are idle. Constraints worth naming: everything under test must live inside the bubble (no real network/OS timers crossing the boundary), and it composes with `-race`. This kills the "sleep 50ms and hope" pattern and the 1ms-tolerance assertions that fail on loaded CI machines.