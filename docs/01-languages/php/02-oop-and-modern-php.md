...
final class User {
    ...
    public private(set) string $displayName;   // asymmetric visibility (8.4)
...
- Property hooks and asymmetric visibility (both 8.4) reduce boilerplate
  for computed/controlled properties; `clone with` (8.5) gives readonly
  classes a declarative wither.
...
since 8.4 properties can be **asymmetrically** visible (`public private(set)
$prop`; 8.5 extends this to static properties). Inside a class, `private`
means *this class only*, not "this object"
...
Native lazy objects defer a real object's initialization until first use —
what Doctrine and Symfony previously did with generated proxy classes.

```php
$r = new ReflectionClass(HeavyService::class);

// Ghost: the object initializes itself in place on first property access.
$service = $r->newLazyGhost(function (HeavyService $s) use ($container): void {
    $s->__construct($container->get(Config::class));
});

// Proxy: a stand-in that forwards to a real instance built by the factory.
$service = $r->newLazyProxy(fn(): HeavyService => HeavyServiceFactory::build());
```

Two strategies to contrast in an interview: **ghosts** (same object, fills
itself in; requires you to control the class) vs **virtual proxies** (factory
may return a pre-existing or differently-sourced instance; one extra
indirection per access). DI containers use them so injecting an expensive
service into every controller costs nothing until a code path actually
touches it; ORMs use them for uninitialized associations. The win over
pre-8.4: no generated proxy code, `instanceof`/reflection behave correctly,
and `readonly`/typed properties work.
...
- **PSR-6** — caching (`CacheItemPoolInterface`); **PSR-16** — simple cache
  (`CacheInterface`).
...
- `#[\Deprecated]` (8.4+) — marks a function, method, or class constant
  deprecated with an optional message/since; the engine raises
  `E_USER_DEPRECATED` on use, so userland deprecations finally behave like
  core ones. PHP 8.5 extends it to traits.
- `#[\NoDiscard]` (8.5+) — warns when a call's return value is ignored
  (think `Result`-style APIs, `fwrite` return codes); an explicit `(void)`
  cast suppresses it intentionally.
- `#[\AllowDynamicProperties]` — relic for legacy code; dynamic
  properties are disabled by default in 8.2+.

Since PHP 8.5, attribute arguments may also contain **static closures and
first-class callables** (constant expressions were extended), which removes a
long-standing limitation for validation/routing attributes.
