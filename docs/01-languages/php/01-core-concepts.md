...
PHP 8.5 added `array_first` and `array_last`:
...
$head = array_first($items);   // 8.5: first value or null; no reset() pointer games
$tail = array_last($items);    // 8.5: last value or null; no end()/key() dance
```
...
`|>` passes the left-hand value into a single-parameter callable on the
right, so nested calls read forwards instead of inside-out:
...
$slug = $title
    ...
    |> strtolower(...)
    |> (fn($s) => str_replace(' ', '-', $s));
// vs str_replace(' ', '-', strtolower(trim($title)))
```

It pairs naturally with first-class callable syntax (`trim(...)`). Interview
framing: it's syntax for left-to-right function composition — no laziness, no
collections magic — and it makes small transformation pipelines readable
without a framework `Collection` wrapper.
...
PHP 8.5 completes the story with **clone with**: `clone($obj, ['prop' =>
$new])` clones and overrides properties in one expression — the declarative
"wither" for readonly value objects that previously needed a hand-written
`withX()` method around `__clone`:

```php
$discounted = clone($price, ['cents' => $price->cents - 100]);
```
...
class Temperature {
    public function __construct(
        public private(set) float $celsius,    // asymmetric visibility (8.4) on a promoted prop
    ) {}
...
- Hooks are independent of asymmetric visibility; both landed in 8.4 and you
  can use them together for a public read / private write property with
  validation on the set hook.
...
set" handwritten properties and is the idiomatic way in 8.4+ to expose
read-only-to-the-outside state. PHP 8.5 extends asymmetric visibility to
**static** properties as well.
...
Before 8.4, the same effect required either a private backing property plus a
public accessor method, or (transitionally) a virtual property hook whose
`get` body wrapped a private field while leaving writes scoped to that field:
...
Asymmetric visibility makes the intent declarative for the common
case. It works with **readonly**: readonly restricts
