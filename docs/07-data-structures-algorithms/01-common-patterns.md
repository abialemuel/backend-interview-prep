...
**Recognize it by:** "merge overlapping intervals", "insert an interval", "meeting rooms — can attend all / minimum rooms needed", "non-overlapping intervals", "employee free time", "car pooling". The input is a list of `[start, end]` pairs and the question is about overlap, coverage, or gaps.

**Core idea:** Almost always: **sort by start**, then sweep once. Two intervals `a` and `b` (with `a.start <= b.start`) overlap iff `b.start <= a.end`. From that one test you get all the variants: merging (extend the current merged end while overlaps continue), counting removals (greedily keep the interval with the smaller end), and detecting conflicts (any overlap at all). The other tool is the **sweep line / min-heap of end times**: for "minimum meeting rooms," sort by start and push end times into a min-heap, popping every end that finishes before the next start — the peak heap size is the answer. (Equivalent formulation: +1/-1 events sorted by time, track the running maximum — which is the difference-array idea from pattern 11 applied to time.)

**Complexity:** O(n log n) for the sort, O(n) sweep. O(n) space (output or heap).

**Backend relevance:** Maintenance windows, on-call schedule overlap, rate-limit windows, booking/reservation conflicts, alert deduplication — interval logic shows up constantly in scheduling-shaped backend code.

**Canonical problem:** *Merge Intervals*; *Insert Interval*; *Meeting Rooms II* (min-heap variant); *Non-overlapping Intervals* (greedy variant).
