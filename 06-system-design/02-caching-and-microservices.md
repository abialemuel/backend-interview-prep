...
A single hot key expires and a burst of traffic immediately hits the DB. Mathematically identical to a stampede butting on a single key, but the term usually implies the key was hot *before* expiry as well.
...
Most production systems use **orchestration for the main business workflow** (order, payment, shipping — Temporal, AWS Step Functions, Cadence) and **choreography for side-effects** (publish `OrderPlaced`, let whoever cares react). Both need idempotency.
...
A queue delivers **at-least-once** (sometimes exactly-once, but assume at-least-once and design your consumers accordingly). Network failures cause same. **Idempotent consumers are mandatory.**
...
Side benefits: you get an event log for free, you can replay, and you avoid dual-write consistency bugs entirely. Industry standard.
...
Trade-off: a BFF is more servers to maintain, but it pays for itself the moment a mobile team can ship without coordinating with the web team. Some shops run BFFs as Lua/glue/GraphQL in the API gateway itself.
...
Both are powerful but introduce real complexity. Use them when the problem demands it; do not adopt them by default.
