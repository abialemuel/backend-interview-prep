...
`dispatchSync`, `dispatchNow` (deprecated), and `afterResponse()` give synchronous or end-of-request behavior.
...
`balance: 'simple'` (default), `'auto'` (uses past wait times), or `false` (fixed counts). Horizon auto-pauses slow queues to prioritize busy ones.
...
Broadcasting pushes server events to browser clients over WebSockets. Drivers: `log`, `null`, `pusher`, `ably`, `redis`, and `reverb` (the first-party WebSocket server introduced in 11.x).
...
In Laravel 11+ the scheduler is configured in `routes/console.php` via the `Schedule` facade or closure passed to `->withSchedule`. In 11.x you may also use a `Schedule::run` invocation. The system relies on a single cron entry:
...
Schedule::command('reports:generate')
    ...
    ->gracePeriod(5);   // minutes after the scheduled time to still run after a deploy/restart
```
