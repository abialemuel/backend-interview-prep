...
**Answer:** `RefreshDatabase` runs migrations once per test run (skipping them if already migrated) and wraps each test in a transaction that rolls back on completion, so tests start with a clean schema and clean data. `DatabaseTransactions` only wraps the test in a transaction and assumes the schema is already migrated; it is faster but can't recover from migration mutations. Use `RefreshDatabase` for tests that touch schema (migrations, custom columns) or want a guaranteed clean slate, and `DatabaseTransactions` when migrations are expensive and you only insert/update rows. Laravel 11 also added `DatabaseTruncation` for cases where transactions don't work (concurrent connections, DDL inside the test).
...
### Q21: How do retries, backoff, and failed jobs work in Laravel queues?
...
### Q22: What does Laravel Horizon add, and how do you operate it in production?
...
### Q23: Explain Laravel broadcasting: channels, drivers, and the auth flow.
...
### Q24: Why do `withoutOverlapping()` and `onOneServer()` matter for scheduled tasks?
...
### Q25: Compare Gates and Policies. When do you use each?
...
### Q26: Should APIs be CSRF-protected? How does Laravel handle this?
...
### Q27: Compare Sanctum and Passport. Which would you choose and when?
...
### Q28: How do config and route caching improve performance, and what are the gotchas?
...
### Q29: What is a terminable middleware and how does it relate to `App::terminating()`?
...
### Q30: How do nested database transactions and `DB::afterCommit()` work?
