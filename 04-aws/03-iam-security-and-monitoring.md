...
- **Users** — long-term identity with credentials (password for console,
  access key pair for API). Avoid programmatic access keys; prefer roles
...
- **AssumeRoleWithWebIdentity** — for OIDC web IdPs (deprecated in favor
  of IAM Identity Center for human SSO, still used for federation flows).
...
Symmetric and asymmetric CMKs (Customer Master Keys) backed by FIPS-
validated HSMs (shared single-tenant HSM partitions) or, for the
highest compliance bar, **Custom Key Store** backed by your own
CloudHSM cluster.
...
**Quotas** — region quotas on **Decrypt/GenerateDataKey** calls; Sonnet
defaults around 5500–50000 RPS depending on region/key type. S3
...
- **Sampling rules** — control the percent of requests traced to bound
  ...
  services.
- **CloudWatch ServiceLens** — combines X-Ray traces with CloudWatch
  Service Map and Logs Insights for "click into slow node → see logs for
  that trace".
- **OpenTelemetry** — AWS Distro for OpenTelemetry (ADOT) is the
  forward path; emit OTel from your app, ship to X-Ray (via collector)
  or to Datadog/Jaeger/Honeycomb. Prefer OTel for new code.
...
Kinesis, ECS task, Batch, API Destination (HTTP webhook),_RED/Batch.
**Schedule rules**: **EventBridge Scheduler** (newer) — name, schedule
(cron/rate/at), target, flexible window, TZ, state — superseds the
older scheduled-rules API and supports one-time schedules with up to a
year in the future.
...
4. **Graviton** — `c7g`/`m7g`/`r7g`/`t4g` ARM: ~20% lower price, often
   +10-20% perf → 25-40% better price/perf for compatible workloads.
   Compile-once, run cheaper. Re-check that your app's native libs / JVM
   / Go / Python have ARM builds.
